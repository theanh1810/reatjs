const Review = ({ review }) => {
  return (
    <div class="app1">
      <h1 id="message">Our Reviews</h1>
      <div class="ReviewCard">
        <div class="Review">
          <div class="Author">
            <img src={review.avatar} alt="anh ava" />
            <div class="Info">
              <div class="Name">{review.name}</div>
              <div class="Position">{review.job}</div>
            </div>
          </div>
          <div class="Content">{review.content}</div>
        </div>
        <div class="Control">
          <button class="btnLeft" onClick={decrement}>
            &lt;
          </button>
          <div>{count}</div>
          <button class="btnRight" onClick={increment}>
            &gt;
          </button>
        </div>
      </div>
...
const App = () => {
  ...
  const review = reviews[index];
    ...
    const handleNext = () => {
      if(index == 0) setIndex(review)
  }

  const style = { color: count > 0 ? "blue" : count < 0 ? "red" : "#333333" };
